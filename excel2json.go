...
func readExcelData(path string) {
	...
	s, keys := excelToSlice(path) // s is a slice of maps
	if s == nil {
		...
		// check PTID
		checkPTID(path, keys)
		checkStatus(path, keys)
		for i, m := range s {
			// check PTID
			matched, err := MatchString("(0?[1-9]|1[012])(0?[1-9]|[12][0-9]|3[01])", m[id1][4:8]) // validta MM AND dd OF A PTID
			if m[id1] != m[id2] {
				newError(path, "", i, "follow_up", "PTID", "PTIDs are different!")
			} else if !matched {
				e.Println(path, "Row #: ", i, "INFO: invaid PTID value!")
			}
      // Check STATUS
			if m[s1]!=m[s2]{
					assignNonEmptyStatus(path, i, m[s1] , m[s2])
			}
			// Event follow_up
			if m["FU_D"] != "" {
				fU := followUp{
			...
			if m["FUREOP_D"] != "" {
				...
				if m[s1] == "R" && (re.Code != 1 || m["FUREOP_D"] == ""){
					e.Println(path, "PTID: ", m[id1], "row #: ",i, "INFO: iNCORRECT INFO OF REOPERATION!")
				}
				writeTOFile(re)
