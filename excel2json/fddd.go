	...
	for j, s := range slices {
		if s == nil {
			...
			for i, m := range s {
				...
				if S1 != "L" && S2 != "L" {
					...
					if est == 0 || est == 1 {
						...
						if diffStatus {
							...
							fu.Fix = append(fu.Fix, msg)
							if helper.StringInSlice(1, S1, codes[4:6]) && helper.StringInSlice(1, S2, codes[:4]) {
								fu.Status = &S2
							}
						}
				...
				if S1 == "L" && !helper.StringInSlice(1, S2, codes[:2]) || (S2 == "L" && !helper.StringInSlice(1, S1, codes[:2])) {
