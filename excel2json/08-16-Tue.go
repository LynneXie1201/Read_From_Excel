			...
			for i, m := range s {
				...
				if lkaEst == 0 || lkaEst == 1 {
					...
					if m["FU_D"] != "" {
						...
						// if one status is "L", add fix messages
						if S1 == "L" || S2 == "L" {
							// add fix messages
							msg := errMessage{"last_known_alive", "conflict of 'L' status and 'LKA date' – has patient been recovered?"}
							lka.Fix = append(lka.Fix, msg)
						}
					...
					if m["FU_D"] != "" {
						...
						if S1 == "L" || S2 == "L" {
							f.Msg = "last_known_alive date with invalid format: '" + lkaDate +
								"' , conflict of 'L' status and 'LKA date' – has patient been recovered?"
						} else {
							f.Msg = "LKA Date with invalid format: '" + lkaDate + "'"
						}
					...
					if est == 0 || est == 1 {
						...
						if statusEst == 0 || statusEst == 1 {
							...
							f := general{
								...
								Date:    "1900-02-02",
								DateEst: 1,
								Msg: "Invalid STATUSDATE: '" + statusDate + "', Notes: '" + m["FU NOTES"] +
							...
							if fuEst == 0 || fuEst == 1 {
								// if FU_D is valid and LKA_D is missing, create a lost_to_followup event,
								...
								if m["LKA_D"] == "" {
									...
									// else if FU_D is valid but LKA_D exists,
									// then create a fix event with date "1900-02-02"
								} else {
									f := general{
										PTID:    ID1,
										Type:    "fix",
										Date:    "1900-02-02",
										DateEst: 1,
										Msg: "followup date and last_known_alive date both exist, Notes: '" + m["FU NOTES"] +
											" " + m["NOTES"] + " " + m["STATUS=O REASON"] + "'",
										Source: source{Type: "followup", Path: []string{}}}
								...
								f := general{
									...
									Date:    "1900-02-02",
									DateEst: 1,
									Msg: "Invalid followup date: '" + fuDate + "', Notes: '" + m["FU NOTES"] +
										" " + m["NOTES"] + " " + m["STATUS=O REASON"] + "'",
									Source: source{Type: "followup", Path: []string{}}}
						...
						f := general{
							...
							Date:    "1900-02-02",
							DateEst: 1,
							Msg: "Invalid STATUS=L DATE: '" + date + "', Notes: '" + m["FU NOTES"] +
								" " + m["NOTES"] + " " + m["STATUS=O REASON"] + "'",
							Source: source{Type: "followup", Path: []string{}}}
