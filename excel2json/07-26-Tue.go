...
func (a followUp) CompareFollowUps(s []followUp) bool {
	for _, b := range s {
		if &a == &b {
			...
			a.LostOnDate == b.LostOnDate && reflect.DeepEqual(a.Fix, b.Fix) &&
...
func (a death) CompareDeath(s []death) bool {
	...
	for i, b := range s {
		if &a == &b {
			...
			a.PrmDth == b.PrmDth && a.Reason == b.Reason && reflect.DeepEqual(a.Fix, b.Fix) {
			b.Source.Path = append(b.Source.Path, a.Source.Path[0])
			return true
...
func (a tia) CompareTia(s []tia) bool {
	for _, b := range s {
		if &a == &b {
			...
			a.Outcome == b.Outcome && a.PTID == b.PTID && reflect.DeepEqual(a.Fix, b.Fix) {
			b.Source.Path = append(b.Source.Path, a.Source.Path[0])
			return true
		}
	}
...
func (a sbe) CompareSbe(s []sbe) bool {
	for _, b := range s {
		if &a == &b {
			...
			a.PTID == b.PTID && reflect.DeepEqual(a.Fix, b.Fix) {
			b.Source.Path = append(b.Source.Path, a.Source.Path[0])
			return true
		}
	}
...
func (a general) CompareEvents(s []general) bool {
	for _, b := range s {
		if &a == &b {
			...
			a.Notes == b.Notes && reflect.DeepEqual(a.Fix, b.Fix) {
			b.Source.Path = append(b.Source.Path, a.Source.Path[0])
			return true
		}
	}
