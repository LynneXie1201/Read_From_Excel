		...
		if s == nil {
			...
			for i, m := range s {
				...
				fuNotes := "Status:" + S1 + ", Notes: " + m["FU NOTES"] + " " +
					m["NOTES"] + " " + m["STATUS=O REASON"] + ", Plat: " + m["PLAT"] +
					", COAG: " + m["COAG"] + ", PO_NYHA: " + m["PO_NYHA"]
				...
				if est == 0 || est == 1 {
					...
					f.Notes = "followup event with invalid date: " + date + ", " + fuNotes
				...
				if S1 == "L" || S2 == "L" {
					...
					if est != 3 {
						...
						f := general{
							...
							DateEst: 1,
							Notes:   "lost_to_followup: " + m["FU NOTES"] + "; " + m["NOTES"] + "; " + m["STATUS=O REASON"],
							Source:  source{Type: "followup", Path: []string{}}}
						f.Source.Path = append(f.Source.Path, path)
				...
				if m["DATEOR"] != "" {
					operDate, _ := helper.CheckDateFormat(e, path, j, i, "DATEOR", m["DATEOR"])
					when = helper.CompareDates(e, date, operDate) //strconv.Itoa(
				} else {
					whenMsg = "when: cannot compare with DATEOR, it is empty or has different name."
				...
				if est == 0 || est == 1 {
					...
					if m["TE1"] == "2" {
						...
						f.Notes = "stroke with no date but code exists, outcome: " + m["TE1_OUT"] +
							", agents: " + m["ANTI_TE1"] + ", when: " + strconv.Itoa(when) + " ." + whenMsg
						if !f.CompareEvents(allFix) {
					...
					if m["TE1"] == "2" || m["TE1"] == "3" {
						...
						if m["TE1"] == "2" {
							...
							f.Notes = "stroke with invalid date format: " + date + ", outcome:" +
								m["TE1_OUT"] + ", agents: " + m["ANTI_TE1"] + ", when: " + strconv.Itoa(when) + " ." + whenMsg
						} else if m["TE1"] == "3" {
				...
				if est == 0 || est == 1 {
					...
					if m["TE2"] == "2" {
						...
						f.Notes = "stroke with no date but code exists, outcome: " +
							m["TE2_OUT"] + ", agents: " + m["ANTI_TE2"] + ", when: " + strconv.Itoa(when) + " ." + whenMsg
						if !f.CompareEvents(allFix) {
				...
				if est == 0 || est == 1 {
					...
					if m["TE3"] == "2" {
						...
						f.Notes = "stroke with no date but code exists, outcome: " +
							m["TE3_OUT"] + ", agents: " + m["ANTI_TE3"] + ", when: " + strconv.Itoa(when) + " ." + whenMsg
						if !f.CompareEvents(allFix) {
